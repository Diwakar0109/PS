    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/80">
      ...
      >
        ...
        <div className="flex justify-center gap-4">
          {showCancel && (
            <button className="px-6 py-2 rounded-md font-semibold text-gray-700 bg-gray-200" onClick={onCancel}>
              Cancel
            </button>
          )}
          <button className="px-6 py-2 rounded-md font-semibold text-white bg-blue-600 " onClick={onConfirm}>
...
const CodeCell = ({ question, mainTask, cellCode, onCodeChange, onRun, onValidate, cellResult, isExecuting, isValidated, customInput, onCustomInputChange, isCustomInputEnabled, onToggleCustomInput, isSessionReady }) => {
...
const ExamPage = () => {
  ...
  const handleSubmitExam = useCallback(async () => {
    ...
    const answers = examParts.map((p) => ({ questionId: p.taskId || p.id, partId: p.part_id || null, code: allCode[p.id] || "", passed: !!validationStatus[p.id] }));
