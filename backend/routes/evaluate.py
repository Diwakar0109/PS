...
import time
import tempfile
import re  # For numerical evaluation
from pathlib import Path
from datetime import datetime
from flask import Blueprint, request, jsonify
from typing import Dict, Tuple

# Jupyter and Pandas imports
...
def extract_and_compare_value(student_output: str, label: str, expected_value: float, tolerance: float) -> Tuple[bool, str]:
    """Finds a label in text, extracts the following number, and compares it to an expected value."""
    try:
        pattern = re.compile(re.escape(label) + r'\s*(-?[\d\.]+)')
        match = pattern.search(student_output)
        if not match:
            return False, f"Failed. The required label '{label}' was not found in the output."
        extracted_string = match.group(1)
        extracted_value = float(extracted_string)
        if abs(extracted_value - expected_value) <= tolerance:
            return True, f"Passed. Found value {extracted_value:.4f} is within tolerance."
        else:
            return False, f"Failed. Found value {extracted_value:.4f}, expected around {expected_value}."
    except ValueError:
        return False, f"Failed. Found label '{label}', but could not parse '{extracted_string}' as a number."
...
def check_keywords_in_text(student_output: str, keywords_str: str, threshold: float = 0.8) -> Tuple[bool, str]:
    """Checks for the presence of keywords in the student's output."""
    ...
    if match_ratio >= threshold:
        return True, f"Passed ({match_ratio:.0%})"
    else:
...
def compare_csvs(student_path: Path, solution_path: Path, key_columns=None, threshold: float = 0.9) -> Tuple[bool, float]:
    """Compares student and solution CSV files."""
    try:
...
def run_code_on_kernel(kc: KernelClient, code: str, timeout: int = 45) -> Tuple[str, str]:
    """Executes code on a Jupyter kernel and captures output."""
    msg_id = kc.execute(code); stdout, stderr = [], []; start_time = time.monotonic()
...
def validate_cell():
    ...
    try:
        if task_type == "csv_similarity":
            print(f"--- Starting CSV Validation for Part: {p_id} ---")
            placeholder = part_data['placeholder_filename']
            solution_path = Path(part_data['solution_file'])

            # 🔹 Store student submissions inside user_generated/<username>
            username = data.get("username", "default_student")
            student_dir = Path(__file__).parent.parent / "data" / "user_generated" / username
            student_dir.mkdir(parents=True, exist_ok=True)
            student_path = student_dir / "submission.csv"

            # Replace placeholder filename with per-student submission path
            modified_code = code.replace(
                f"'{placeholder}'", f"r'{student_path.as_posix()}'"
            ).replace(
                f"\"{placeholder}\"", f"r'{student_path.as_posix()}'"
            )
            ...
            if stderr:
                print(f"  - ERROR: Student code failed.\n{stderr}")
                test_results.append(False)
            else:
                # 🔍 Debug print
                print(f"🔍 Comparing student file: {student_path}")
                print(f"   with solution file: {solution_path}")

                passed, score = compare_csvs(
                    student_path,
                    solution_path,
                    part_data.get('key_columns'),
                    part_data.get('similarity_threshold', 0.9)
                )

                if not passed:
                    try:
                        import pandas as pd
                        print("\n📄 DEBUG: First 5 rows of student CSV:")
                        print(pd.read_csv(student_path).head())
                        print("\n📄 DEBUG: First 5 rows of solution CSV:")
                        print(pd.read_csv(solution_path).head())
                    except Exception as e:
                        print(f"⚠️ Could not preview CSVs: {e}")

                print(f"  - Result: {'PASSED' if passed else 'FAILED'}. (Similarity: {score:.2f})")
                test_results.append(passed)

        elif task_type == "text_similarity":
            print(f"--- Starting Text Validation for Part: {p_id} ---")
            stdout, stderr = run_code_on_kernel(kc, code)
            if stderr:
                print(f"  - ERROR: Student code failed.\n{stderr}"); test_results.append(False)
            else:
                passed, reason = check_keywords_in_text(stdout, part_data.get("expected_text", ""), part_data.get('similarity_threshold', 0.8))
                print(f"  - Result: {'PASSED' if passed else 'FAILED'}. ({reason})"); test_results.append(passed)

        elif task_type == "numerical_evaluation":
            print(f"--- Starting Numerical Validation for Part: {p_id} ---")
            stdout, stderr = run_code_on_kernel(kc, code)
            if stderr:
                print(f"  - ERROR: Student code failed.\n{stderr}"); test_results.append(False)
            else:
                passed, reason = extract_and_compare_value(
                    student_output=stdout,
                    label=part_data.get("evaluation_label"),
                    expected_value=float(part_data.get("expected_value")),
                    tolerance=float(part_data.get("tolerance"))
                )
                print(f"  - Result: {'PASSED' if passed else 'FAILED'}. ({reason})"); test_results.append(passed)
        else:
        ...
        print(f"CRITICAL VALIDATION ERROR: {e}"); test_results.append(False)
    
    return jsonify({"test_results": [bool(x) for x in test_results]})

@evaluation_bp.route('/run', methods=['POST'])
def run_cell():
    data = request.get_json(); session_id, student_code, user_input = data.get('sessionId'), data.get('cellCode', 'pass'), data.get('userInput', '')
    if not session_id or session_id not in USER_KERNELS: return jsonify({'error': 'User session not found or invalid.'}), 404
    _km, kc = USER_KERNELS[session_id]
    full_script = f"import builtins\n_input_lines = {json.dumps(user_input)}.splitlines()\n_input_lines.reverse()\ndef _mock_input(prompt=''):\n    try: return _input_lines.pop()\n    except IndexError: return ''\nbuiltins.input = _mock_input\n{student_code}"
    try:
        stdout, stderr = run_code_on_kernel(kc, full_script)
...
def submit_answers():
    data = request.get_json(); session_id, username, subject, level, answers = data.get('sessionId'), data.get('username'), data.get('subject'), data.get('level'), data.get('answers', [])
